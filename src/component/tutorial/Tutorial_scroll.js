...
export default function TutorialScroll() {
  ...
  const { title1, desc1, title2, desc2, title3, desc3, title4, desc4 } = LangTutorial;
        ...
        {imageArr.map((image) => (
          <ContentBox key={image.id} {...image.animation} data-set={image.id}>
            <ImageBox color={image.color}>
              {Number(image.animation.trigger) === image.id && image.last ? (
                <EpicImg />
              ) : (
                <Image change={image.mobile} src={image.img} value={image.id} onContextMenu={(e) => e.preventDefault()} type="video/mp4" autoPlay={true} loop muted />
              )}
            </ImageBox>
            {image.last ? (
              <DescriptBox color={image.color}>
                ...
                <ButtonWrap>
                  <Button styling={'signup'} onClick={() => goURL({ pathname: '/login', state: { sign: true } })}>
                    {image.desc.join}
                  </Button>
                  <Button styling={'login'} onClick={() => goURL({ pathname: '/login' })}>
                    {image.desc.Login}
                  </Button>
                </ButtonWrap>
