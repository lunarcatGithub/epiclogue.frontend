...
export const Develop201212 = {
  ...
  developSub201212: {
    2: <UL>The way you write your feedback has changed. Below are the added patches!
    ...
    0: <UL>피드백 작성 방식이 변경되었어요 아래는 추가된 패치 내용이랍니다!
    ...
    1: <UL>フィードバックの作成方式が変更されました。 以下は追加されたパッチ内容です
...
export const Develop201130 = {
  ...
  developSub201130: {
    2: <UL>❗In this patch, We hid the header while scrolling so that you can focus more on posts We also blocked the background from scrolling when it was popup😉 Now You can't see the deleted contents and private posts!</UL>,
    0: <UL>❗이번 패치에서 콘텐츠 감상에 집중할 수 있게 헤더를 스크롤 할 때는 숨겨놓았어요👍 또 팝업 띄울 때 백그라운드가 스크롤 되지 않도록 막았답니다😉 삭제된 콘텐츠와 비공개 게시물은 이제 보이지않아요!</UL>,
    1: <UL>❗今回のパッチで作品の感想に集中できるように、ヘッダーをスクロールするときは隠しておきました👍 またポップアップ表示の時、バックグラウンドが スクロールされないように防ぎました😉 削除されたコンテンツと非公開投稿はもう見えません!</UL>
  },
};
...
export const Develop201126 = {
    ...
    developSub201126: {
      2: <UL>✨We solved the error of not being able to sign up by browser when registering as a member! We also improved the feedback viewing function and loading bar layout🎉 You can put your long-awaited contents on Myboard! Finally, you're able to create a second one, so please use it a lot😘</UL>,
      0: <UL>✨회원가입시 브라우저별 가입이 안되던 오류를 해결했어요 그리고 피드백 더보기 기능과 로딩바 레이아웃을 개선했답니다🎉 대망의 마이보드에 콘텐츠를 담을 수 있게 되었어요! 마지막으로 2차 창작이 가능해졌으니 많은 이용 부탁드려요😘</UL>,
      1: <UL>✨会員登録時にブラウザ別に登録ができなかったエラーを解決しました。 そして、フィードバックザ表示機能とローディングバーレイアウトを改善しました🎉 待ちに待ったマイボードにコンテンツを保存できるようになったんです! 最後に2次創作が可能になったので 多くの利用をお願いします😘</UL>
    },
  };
  ...
  export const Develop201116 = {
    ...
    developSub201116: {
      2: <UL>Welcome to EpicLogue!🙆‍♂️ All functions under development or functions to be developed with feedback will be recorded in the development note. We will do our best to make it a service that grows together🥰</UL>,
